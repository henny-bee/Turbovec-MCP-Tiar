...
def test_deterministic_vector_id_mapping(db):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT vector_id FROM vector_id_mapping WHERE node_id = 'colliding-node'")
...
def test_node_crud(db):
    ...
    cursor.execute("SELECT name, description FROM entities_fts WHERE entity_id = ?", (node_id,))
    ...
    assert json.loads(row[1]) == {"description": "Updated description", "status": "inactive"}
    ...
    cursor.execute("SELECT COUNT(*) FROM vector_id_mapping WHERE node_id = ?", (node_id,))
    assert cursor.fetchone()[0] == 0
...
def test_observation_crud_and_fts_sync(db):
    ...
    db.create_node(node_id=node_id, name="Entity Node", node_type="entity", properties={"description": "Main entity"})
    ...
    cursor.execute("SELECT name, observations FROM entities_fts WHERE entity_id = ?", (node_id,))
    fts_row = cursor.fetchone()
    assert fts_row is not None
    ...
    cursor.execute("SELECT observations FROM entities_fts WHERE entity_id = ?", (node_id,))
    fts_row = cursor.fetchone()
