...
async def test_add_observation_tool(mcp, db):
    ...
    res_obs = add_observation_fn(entity_id="ent-obs", content="This is an interesting fact.")
...
async def test_create_and_delete_relationship_tools(mcp, db):
    ...
    # Check SQLite
    cursor = db.conn.cursor()
    cursor.execute("SELECT relationship_type, weight FROM edges WHERE from_node_id = 'n1'")
    row = cursor.fetchone()
    assert row[0] == "CONNECTS_TO"
    assert row[1] == 0.5

    # Delete relationship
    res_del_rel = delete_relationship_fn(from_node_id="n1", to_node_id="n2", relationship_type="CONNECTS_TO")
...
async def test_get_hologram_and_neighbors_tools(mcp, db):
    ...
    create_relationship_fn(from_node_id="anchor", to_node_id="friend", relationship_type="KNOWS")
...
async def test_session_and_breakthrough_tools(mcp, db):
    ...
    # Start session
    sid = start_session_fn(session_id="sess-01", name="Session 01", properties={"focus": "test"})
    assert sid == "sess-01"

    # Record breakthrough
    res_bt = record_breakthrough_fn(session_id="sess-01", title="Aha Moment", content="I figured it out!")
...
async def test_semantic_radar_tool(mcp, db):
    ...
    with patch.object(db.model, "encode", return_value=vec):
        ...
        assert {gaps[0]["node_1"]["id"], gaps[0]["node_2"]["id"]} == {"node-1", "node-2"}
...
async def test_add_knowledge_pipeline_integration(mcp, db):
    ...
    text = "John Doe works at Google. Python is a programming language. I prefer VS Code."
    ...
    cursor.execute("SELECT relationship_type FROM edges WHERE from_node_id = ? AND to_node_id = ?", (john_row[0], google_row[0]))
...
async def test_relationship_inference_and_auto_creation(mcp, db):
    ...
    create_entity_fn = mcp._tool_manager._tools["create_entity"].fn
    run_relationship_discovery_fn = mcp._tool_manager._tools["run_relationship_discovery"].fn
    ...
    with patch.object(db.model, "encode", return_value=vec):
        ...
        results = run_relationship_discovery_fn(similarity_threshold=0.9, project_id="default", auto_create=False)
        ...
        results_with_create = run_relationship_discovery_fn(similarity_threshold=0.9, project_id="default", auto_create=True)
        ...
        cursor.execute("SELECT relationship_type FROM edges WHERE from_node_id = 'p-1' AND to_node_id = 's-1'")
        row = cursor.fetchone()
...
async def test_new_parity_tools_and_wrappers(mcp, db):
    ...
    with db.conn:
        db.conn.execute("UPDATE nodes SET created_at = ?, occurred_at = ? WHERE id = 'n-time1'", (t1, t1))

    create_entity_fn(id="n-time2", name="Temporal Entity 2", node_type="entity")
    with db.conn:
        db.conn.execute("UPDATE nodes SET created_at = ?, occurred_at = ? WHERE id = 'n-time2'", (t2, t2))

    add_observation_fn(entity_id="n-time1", content="Fact registered at t2.")
    with db.conn:
        db.conn.execute("UPDATE observations SET created_at = ?, occurred_at = ? WHERE entity_id = 'n-time1'", (t2, t2))
    ...
    create_relationship_fn = mcp._tool_manager._tools["create_relationship"].fn
    create_relationship_fn(from_node_id="n-time1", to_node_id="n-time2", relationship_type="RELATED_TO")
    ...
    with db.conn:
        db.conn.execute("UPDATE nodes SET status = 'STALE', created_at = '2020-01-01T00:00:00Z' WHERE id = 'n-time2'")
