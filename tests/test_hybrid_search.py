...
def test_fts_only_search(db):
    ...
    db.create_node(
        node_id="node-c", name="Database Systems and SQL", node_type="concept"
    )
    ...
    assert (
        results[0]["rrf_score"] == 1.0 / 61.0
    )  # 1-based rank 1 in lexical, 0 in vector
...
def test_vector_only_search(db, mock_sentence_transformer):
    ...
    assert (
        results[0]["rrf_score"] == 1.0 / 61.0
    )  # 1-based rank 1 in vector, 0 in lexical
