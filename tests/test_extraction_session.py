...
def test_extraction_pipeline_fallback(db):
    ...
    with patch.dict("sys.modules", {"spacy": None}):
        ...
        works_at_rel = [r for r in rels if r["from"] == "John Doe" and r["to"] == "Google"]
...
def test_session_management(db):
    ...
    cursor.execute("SELECT name, node_type, properties FROM nodes WHERE id = ?", (session_id_1,))
    row = cursor.fetchone()
    ...
    cursor.execute("SELECT name, node_type, properties FROM nodes WHERE id = ?", (bt_id,))
    row = cursor.fetchone()
    assert row is not None
    ...
    cursor.execute("SELECT from_node_id, to_node_id, relationship_type FROM edges WHERE from_node_id = ?", (bt_id,))
    ...
    # 4. End Session 1
    db.end_session(session_id_1, "Completed initial architecture exploration and WAL benchmark.")
...
def test_point_in_time_queries(db):
    ...
    db.create_edge(edge_id="edge-ab", from_node_id="node-a", to_node_id="node-b", relationship_type="DEPENDS_ON")
    with db.conn:
        db.conn.execute("UPDATE edges SET created_at = ? WHERE id = 'edge-ab'", (t2,))
        
    # Create observation on Node A at t3
    db.create_observation(obs_id="obs-a-1", entity_id="node-a", content="Observation at t3")
    with db.conn:
        db.conn.execute("UPDATE observations SET created_at = ? WHERE id = 'obs-a-1'", (t3,))
