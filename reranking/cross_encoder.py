    ...
    def rerank(self, query: str, candidates: List[Dict[str, Any]], top_k: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for c in candidates:
                ...
                desc = properties.get("description", "") if isinstance(properties, dict) else str(properties)
                obs_text = " ".join([o.get("content", "") for o in c.get("observations", [])])
            ...
            reranked = sorted(candidates, key=lambda x: x.get("rerank_score", -9999.0), reverse=True)
            logger.info(f"Reranking of {len(candidates)} candidates completed in {time.perf_counter() - start_time:.4f}s.")
