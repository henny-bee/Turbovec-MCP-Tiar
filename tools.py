...
def register_tools_and_prompts(mcp: FastMCP, db: VectorDB):
    ...
    def add_observation(
        ...
        try:
            ...
            logger.error(
                f"Error adding observation to entity '{entity_id}': {e}", exc_info=True
            )
            return f"Error: {e}"
    ...
    def create_relationship(
        ...
        logger.info(
            f"Tool 'create_relationship' called: {from_node_id} -> {to_node_id} ({relationship_type})"
        )
        try:
    ...
    def delete_relationship(
        ...
        logger.info(
            f"Tool 'delete_relationship' called: {from_node_id} -> {to_node_id} ({relationship_type})"
        )
        try:
            edge_id = f"edge-{from_node_id}-{to_node_id}-{relationship_type}"
    ...
    def semantic_radar(
        ...
        logger.info(
            f"Tool 'semantic_radar' called with threshold: {similarity_threshold}, auto_create: {auto_create}"
        )
        try:
    ...
    def run_relationship_discovery(
        ...
        logger.info(
            f"Tool 'run_relationship_discovery' called with similarity_threshold: {similarity_threshold}, auto_create: {auto_create}"
        )
        try:
            return db.semantic_radar(
                similarity_threshold=similarity_threshold,
    ...
    def set_background_discovery(
        ...
        logger.info(
            f"Tool 'set_background_discovery' called with enabled: {enabled}, interval_seconds: {interval_seconds}, similarity_threshold: {similarity_threshold}"
        )
        try:
    ...
    def prune_stale(
        ...
        logger.info(
            f"Tool 'prune_stale' called with max_age_days: {max_age_days}, dry_run: {dry_run}"
        )
