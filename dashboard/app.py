...
# Page configuration
st.set_page_config(
    page_title="Turbovec MCP Developer Dashboard",
    page_icon="🧬",
    layout="wide"
)
...
if not os.path.exists(DB_FILE):
    st.error(f"SQLite Database not found at '{DB_FILE}'. Please start the server and run some queries first.")
else:
    # Connect to db
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "📊 Telemetry & Latency", 
        "🕸️ Graph Topology & Communities", 
        "📚 Librarian Cycles", 
        "🕰️ Temporal Changes & Timeline", 
        "🍾 Bottles & Notes"
    ])
    ...
    with tab1:
        ...
        try:
            metrics_df = pd.read_sql_query("SELECT * FROM search_metrics ORDER BY id DESC", conn)
            if metrics_df.empty:
                st.warning("No search metrics recorded yet. Try running some semantic searches via MCP!")
            else:
                ...
                col2.metric("P50 Latency (s)", f"{metrics_df['total_latency'].median():.4f}")
                col3.metric("Max Latency (s)", f"{metrics_df['total_latency'].max():.4f}")
                ...
                avg_latencies = metrics_df[[
                    'fts_latency', 'vector_latency', 'graph_latency', 'embedding_latency', 'reranker_latency'
                ]].mean()
    ...
    with tab2:
        ...
        try:
            nodes_data = pd.read_sql_query("SELECT id, name, node_type, status, created_at, certainty, salience_score, retrieval_count FROM nodes", conn)
            ...
            edges_data = pd.read_sql_query("SELECT id, from_node_id, to_node_id, relationship_type, confidence, weight, created_at FROM edges", conn)
    ...
    with tab3:
        ...
        try:
            runs_df = pd.read_sql_query("SELECT * FROM librarian_runs ORDER BY id DESC", conn)
            if runs_df.empty:
                st.warning("No autonomous Librarian runs recorded yet.")
            else:
                col1, col2, col3 = st.columns(3)
                col1.metric("Total Librarian Runs", len(runs_df))
                col2.metric("Total Synthesized Concepts", int(runs_df['synthesized_concepts'].sum()))
                col3.metric("Total Inferred Relationships", int(runs_df['edges_created'].sum()))
    ...
    with tab4:
        ...
        try:
            nodes_timeline = pd.read_sql_query("SELECT created_at as timestamp, 'Entity Created' as event_type, name || ' (' || node_type || ')' as detail FROM nodes", conn)
            obs_timeline = pd.read_sql_query("SELECT o.created_at as timestamp, 'Observation Recorded' as event_type, n.name || ': ' || o.content as detail FROM observations o JOIN nodes n ON o.entity_id = n.id", conn)
            
            timeline_all = pd.concat([nodes_timeline, obs_timeline]).sort_values(by="timestamp", ascending=False)
    ...
    with tab5:
        ...
        try:
            bottles_df = pd.read_sql_query("SELECT * FROM bottles ORDER BY created_at DESC", conn)
            if bottles_df.empty:
