...
class VectorDB:
    def __init__(
        ...
        if sqlite_db_file == "memory.db" and os.path.dirname(metadata_file):
            self.sqlite_db_file = os.path.join(os.path.dirname(metadata_file), "memory.db")
        else:
    ...
    def _init_sqlite_db(self):
        ...
        with conn:
            ...
            try:
                conn.execute("ALTER TABLE nodes ADD COLUMN status TEXT NOT NULL DEFAULT 'ACTIVE';")
                logger.info("Migrated SQLite schema: added 'status' column to 'nodes' table.")
            ...
            # Create indexes
            conn.execute("CREATE INDEX IF NOT EXISTS idx_nodes_project_type ON nodes(project_id, node_type);")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_nodes_created_at ON nodes(created_at);")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_edges_from ON edges(from_node_id);")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_edges_to ON edges(to_node_id);")
    ...
    def generate_vector_id(self, node_id: str, conn: sqlite3.Connection) -> int:
        ...
        try:
            ...
            candidate_id = int.from_bytes(hash_bytes[:8], byteorder='big') & 0xFFFFFFFFFFFFFFFF

        cursor = conn.cursor()
        while True:
            signed_id = self._to_signed_64(candidate_id)
            cursor.execute("SELECT node_id FROM vector_id_mapping WHERE vector_id = ?", (signed_id,))
            row = cursor.fetchone()
            if row is None:
                return candidate_id
            elif row[0] == node_id:
                    ...
                    for doc_id, doc in self.document_store.items():
                        ...
                        conn.execute(
                            ...
                            (
                                ...
                                created_at,
                                "confirmed",
                                json.dumps({"content": doc.get("content", ""), "chunk_index": doc.get("chunk_index", 0)}),
                            ),
                        )
                        ...
                        conn.execute(
                            ...
                            """,
                            (node_id, "default", doc.get("title", "Untitled"), "document", desc, ""),
                        )
                        conn.execute(
    ...
    def add_knowledge(self, title: str, content: str) -> str:
        ...
        try:
            ...
            logger.error(f"SQLite insertion failed during add_knowledge: {e}", exc_info=True)
        ...
        try:
            ...
            for obs in extracted.get("observations", []):
                cursor.execute("SELECT id FROM nodes WHERE name = ?", (obs["entity_name"],))
            ...
            logger.error(f"Extraction pipeline failed during add_knowledge: {e}", exc_info=True)
        ...
        for doc_id in to_delete:
            if self.index.remove(doc_id):
                ...
                try:
                    ...
                    logger.error(f"Failed to delete {doc_id} from SQLite: {e}", exc_info=True)
    ...
    def update_node(
        ...
        new_salience_score = salience_score if salience_score is not None else current_salience_score
        new_occurred_at = occurred_at if occurred_at is not None else current_occurred_at
    ...
    def delete_node(self, node_id: str) -> bool:
        ...
        cursor = self.conn.cursor()
        cursor.execute("SELECT vector_id FROM vector_id_mapping WHERE node_id = ?", (node_id,))
    ...
    def _sync_node_fts_and_embedding(self, node_id: str, conn: sqlite3.Connection):
        ...
        # 1. Fetch node
        cursor = conn.cursor()
        cursor.execute("SELECT name, node_type, properties FROM nodes WHERE id = ?", (node_id,))
    ...
    def extract_entities_and_relations(self, text: str) -> dict:
        ...
        try:
            ...
            logger.info(f"spaCy not available or failed to load: {e}. Using regex fallback.")

        # Common stop words to ignore as standalone fallback named entities
        STOP_WORDS = {
            "I", "The", "A", "An", "They", "He", "She", "It", "We", "You", 
            "But", "And", "Or", "If", "Then", "Else", "When", "Where", "Why", "How", 
        ...
        for sentence in sentences:
            ...
            if pref_match:
                ...
                pref_item = re.sub(r'^(?:a|an|the)\s+', '', pref_item, flags=re.IGNORECASE)
                if pref_item and len(pref_item) >= 2:
                    if pref_item not in entities:
                        ...
                        entities[pref_item]["properties"]["description"] = "Preferred by user"
                    observations.append({
                        "entity_name": "User",
                        "content": f"Prefers {pref_item}"
                    })
            ...
            rel_patterns = [
                ("works at", r'\b(?P<from_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\s+works?\s+at\s+(?P<to_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\b'),
                ("is part of", r'\b(?P<from_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\s+is\s+part\s+of\s+(?P<to_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\b'),
                ("depends on", r'\b(?P<from_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\s+depends?\s+on\s+(?P<to_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\b'),
                ("member of", r'\b(?P<from_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\s+(?:is\s+)?members?\s+of\s+(?P<to_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\b'),
                ("lives in", r'\b(?P<from_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\s+lives?\s+in\s+(?P<to_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\b'),
                ("created", r'\b(?P<from_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\s+created\s+(?P<to_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\b'),
                ("owns", r'\b(?P<from_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\s+owns?\s+(?P<to_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\b'),
                ("supports", r'\b(?P<from_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\s+supports?\s+(?P<to_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\b'),
                ("is partner of", r'\b(?P<from_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\s+is\s+partners?\s+of\s+(?P<to_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\b'),
                ("is connected to", r'\b(?P<from_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\s+is\s+connected\s+to\s+(?P<to_node>[A-Z][a-zA-Z0-9]*(?:\s+[A-Z][a-zA-Z0-9]*)*)\b'),
            ]
    ...
    def end_session(self, session_id: str, summary: str) -> None:
        ...
        properties["ended_at"] = datetime.datetime.now(datetime.timezone.utc).isoformat()
    ...
    def search_hybrid(
        ...
        try:
            ...
            self.telemetry.record_search(
                ...
                fts_latency=0, vector_latency=0, graph_latency=0,
                embedding_latency=0, reranker_latency=0,
                candidate_count=0, result_count=0,
                is_error=True, error_code=SQLITE_UNAVAILABLE
            )
            raise SearchError(
            ...
            try:
                ...
                if total_nodes > 0:
                    ...
                    if signed_ids:
                        ...
                        node_map = {self._to_unsigned_64(row[1]): row[0] for row in cursor.fetchall()}
                ...
                self.telemetry.record_search(
                    total_latency=time.perf_counter() - start_total,
                    fts_latency=0, vector_latency=t_vec, graph_latency=0,
                    embedding_latency=t_embed, reranker_latency=0,
                    candidate_count=0, result_count=0,
                    is_error=True, error_code=se.code
                )
                raise
            except Exception as e:
                ...
                self.telemetry.record_search(
                    ...
                    embedding_latency=t_embed, reranker_latency=0,
                    candidate_count=0, result_count=0,
                    is_error=True, error_code=MEMORY_LAYER_DEGRADED
                )
                raise SearchError(
            ...
            if safe_query:
                try:
                    ...
                    if lexical_results:
                        ...
                        lexical_results = [r for r in lexical_results if r in active_lex]
                except sqlite3.OperationalError as e:
                    logger.warning(f"FTS search channel failed for query {query!r}: {e}")
        ...
        for rank_idx, node_id in enumerate(vector_results):
            rank = rank_idx + 1
            rrf_scores[node_id] = rrf_scores.get(node_id, 0.0) + w_vector / (60.0 + rank)
            
        # Process Lexical rank scoring
        for rank_idx, node_id in enumerate(lexical_results):
            rank = rank_idx + 1
            rrf_scores[node_id] = rrf_scores.get(node_id, 0.0) + w_lexical / (60.0 + rank)

        if not rrf_scores:
            self.telemetry.record_search(
                total_latency=time.perf_counter() - start_total,
                fts_latency=t_fts, vector_latency=t_vec, graph_latency=0,
                embedding_latency=t_embed, reranker_latency=0,
                candidate_count=0, result_count=0,
                is_error=False
            )
        ...
        sorted_rrf = sorted(rrf_scores.items(), key=lambda x: x[1], reverse=True)[:candidate_limit]
        ...
        try:
            ...
            self.telemetry.record_search(
                ...
                fts_latency=t_fts, vector_latency=t_vec, graph_latency=0,
                embedding_latency=t_embed, reranker_latency=0,
                candidate_count=candidate_count, result_count=0,
                is_error=True, error_code=SQLITE_TRANSACTION_FAILED
            )
            raise SearchError(
        ...
        if self.reranker.enabled:
            ...
            try:
                ...
                self.telemetry.record_search(
                    total_latency=time.perf_counter() - start_total,
                    fts_latency=t_fts, vector_latency=t_vec, graph_latency=t_graph,
                    embedding_latency=t_embed, reranker_latency=0,
                    candidate_count=candidate_count, result_count=0,
                    is_error=True, error_code=RERANKER_FAILED
                )
                raise SearchError(
    ...
    def semantic_radar(self, similarity_threshold: float = 0.7, project_id: str = "default", auto_create: bool = False) -> list[dict]:
        """
            ...
            for j in range(i + 1, len(nodes)):
                ...
                if similarity >= similarity_threshold:
                    if not self._has_path(node_a["id"], node_b["id"]):
                        ...
                        if auto_create:
                            try:
                                edge_id = f"edge-{node_a['id']}-{node_b['id']}-{rel_type}"
                                ...
                                if cursor.fetchone()[0] == 0:
                                    edge_details = self.create_edge(
                                        ...
                                        properties={"reason": "Automatic relationship inference from semantic_radar"}
                                    )
                                    created_edge = True
                            except Exception as e:
                                logger.error(f"Failed to auto-create edge in semantic_radar: {e}")
    ...
    def start_background_discovery(self) -> None:
        ...
        self.bg_thread = threading.Thread(target=self._background_discovery_loop, daemon=True)
        ...
        while not self.bg_stop_event.is_set():
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"Failed to query projects for background discovery: {e}")
                ...
                for proj in projects:
                    if self.bg_stop_event.is_set():
                        return
                    logger.info(f"Scanning project '{proj}' for relationship discovery...")
                    created_results = self.semantic_radar(
                        similarity_threshold=self.bg_similarity_threshold,
                        project_id=proj,
                        auto_create=True
                    )
                    actual_created = sum(1 for item in created_results if item.get("created_edge"))
                    if actual_created > 0:
                        logger.info(f"Background discovery successfully created {actual_created} edges in project '{proj}'.")
            except Exception as e:
                logger.error(f"Error in background relationship discovery loop: {e}", exc_info=True)
    ...
    def get_temporal_neighbors(self, node_id: str, direction: str = "both", depth: int = 1) -> list[dict]:
    ...
    def create_bottle(self, message: str, priority: str = "medium", expires_at: str = None, author_session_id: str = None) -> dict:
        """Creates a standardized bottle note in the database."""
        return self.bottle_manager.create_bottle(message, priority, expires_at, author_session_id)
