...
def prune_stale(db, max_age_days: int, dry_run: bool = False) -> List[Dict[str, Any]]:
    ...
    """
    try:
        limit_date = (datetime.datetime.now(datetime.timezone.utc) - datetime.timedelta(days=max_age_days)).isoformat()
        ...
        if not dry_run and candidates:
            ...
            logger.info(f"Successfully pruned {len(candidates)} stale nodes from database.")
