...
class OntologyManager:
    ...
    def load_ontology(self) -> None:
        ...
        if not os.path.exists(self.filepath):
            logger.info(
                f"Ontology file {self.filepath} not found. Creating default ontology."
            )
        ...
        try:
            with open(self.filepath, "r", encoding="utf-8") as f:
                ...
                self.relations = set(
                    data.get("relations", DEFAULT_ONTOLOGY["relations"])
                )
                logger.info(
                    f"Loaded ontology version {self.version} with {len(self.entities)} entity types and {len(self.relations)} relation types."
                )
        except Exception as e:
            logger.error(
                f"Failed to load ontology from {self.filepath}: {e}. Falling back to default."
            )
            self.entities = set(DEFAULT_ONTOLOGY["entities"])
            self.relations = set(DEFAULT_ONTOLOGY["relations"])
            self.version = DEFAULT_ONTOLOGY["version"]
    ...
    def add_relation_type(self, relation_type: str) -> bool:
        ...
        normalized = (
            relation_type.strip()
        )  # Preserve case for relation types like BREAKTHROUGH_IN
        if not normalized:
            raise SearchError(
                code=ONTOLOGY_VALIDATION_FAILED,
