    ...
    def list_bottles(self, include_acknowledged: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if include_acknowledged:
                cursor.execute(
                    "SELECT id, message, created_at, author_session_id, priority, expires_at, acknowledged FROM bottles ORDER BY created_at DESC"
                )
            else:
                cursor.execute(
                    "SELECT id, message, created_at, author_session_id, priority, expires_at, acknowledged FROM bottles WHERE acknowledged = 0 ORDER BY created_at DESC"
                )
    ...
    def acknowledge_bottle(self, bottle_id: str) -> bool:
        ...
        try:
            with self.db.conn as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE bottles SET acknowledged = 1 WHERE id = ?", (bottle_id,)
                )
