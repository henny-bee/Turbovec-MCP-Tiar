...
def diff_knowledge_state(db, from_timestamp: str, to_timestamp: str) -> Dict[str, Any]:
    ...
    for nid, node in to_nodes.items():
        if nid not in from_nodes:
            ...
            if node.get("updated_at") != old_node.get("updated_at") or node.get(
                "properties"
            ) != old_node.get("properties"):
                changed.append({"id": nid, "before": old_node, "after": node})
...
def get_temporal_neighbors(
    db, node_id: str, direction: str = "both", depth: int = 1
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        cursor.execute(
            "SELECT name, created_at, occurred_at FROM nodes WHERE id = ?", (node_id,)
        )
