    ...
    def run_cycle(self) -> Dict[str, Any]:
        ...
        try:
                            ...
                            duplicates.append(
                                {
                                    "node_1": {
                                        "id": node_ids[idx_a],
                                        "name": node_names[idx_a],
                                    },
                                    "node_2": {
                                        "id": node_ids[idx_b],
                                        "name": node_names[idx_b],
                                    },
                                    "similarity": sim,
                                }
                            )
            ...
            try:
                ...
                discovered_edges_count = sum(
                    1 for r in radar_results if r.get("created_edge")
                )
            except Exception as e:
                logger.error(
                    f"Librarian relationship discovery via semantic_radar failed: {e}"
                )
            ...
            for lbl, indices in clusters.items():
                ...
                if all(t == "concept" for t in types_in_cluster):
                    logger.debug(
                        f"Skipping cluster {lbl} because all members are concepts."
                    )
                    continue
                ...
                cursor.execute(
                    "SELECT id, properties FROM nodes WHERE node_type = 'concept'"
                )
                ...
                properties = {"description": description, "provenance": provenance}
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to create synthesized concept {concept_id}: {e}"
                    )

            duration = (datetime.datetime.now() - start_time).total_seconds()
            logger.info(
                f"Librarian cycle completed in {duration:.4f}s. Synthesized {synthesized_count} concepts."
            )
